...
export async function uploadSlideImage(file, opts = {}) {
  ...
  try {
    const res = await fetch(url, { method: 'POST', body: formData, signal: ctrl.signal, headers: adminToken ? { Authorization: `Bearer ${adminToken}` } : {}, credentials: 'include' });
    if (!res.ok) {
    ...
    if (e.name === 'AbortError') throw new Error('Upload timed out. Try a smaller image or check the server.');
    throw e;
  } finally {
