...
import { isUiMockMode } from '@/lib/uiMockMode';
import {
  getUiMockProducts,
  getUiMockCategories,
  findUiMockProduct,
  getUiMockBlogPosts,
  findUiMockBlogPostBySlug,
  getUiMockSlider,
  getUiMockReviewsForProduct,
  getUiMockContentSettings,
} from '@/lib/uiMockData';
...
export async function getProducts(params = {}) {
  if (isUiMockMode()) {
    return { data: getUiMockProducts({ categoryId: params.categoryId }) };
  }
...
export async function getProductBySlug(slug) {
  if (isUiMockMode()) {
    const p = findUiMockProduct(slug);
    if (!p) {
...
export async function getContentSettings() {
  if (isUiMockMode()) {
    return getUiMockContentSettings();
  }
...
export async function getBlogPosts(params = {}) {
  if (isUiMockMode()) {
    return { data: getUiMockBlogPosts() };
  }
...
export async function getBlogPostBySlug(slug) {
  if (isUiMockMode()) {
    const p = findUiMockBlogPostBySlug(slug);
    if (!p) {
      const err = new Error('Not found');
      err.status = 404;
      throw err;
...
export async function getReviews(productId) {
  if (!productId) return [];
  if (isUiMockMode()) {
    return getUiMockReviewsForProduct(productId);
  }
