...
function getBase() {
  if (typeof window !== 'undefined') {
    ...
    if (host === 'shifaefitrat.com' || host === 'www.shifaefitrat.com') return `https://${host.replace(/^www\./, '')}`;
    ...
    if (onPublicHost && window.location?.origin) {
      const envUrl = (window.__NEXT_PUBLIC_API_URL__ || process.env?.NEXT_PUBLIC_API_URL || '').replace(/\/$/, '');
      if (!envUrl || isLoopback(envUrl)) return window.location.origin;
    }
    if (onPublicHost && window.location?.origin) return window.location.origin;
  }
...
function uploadWithProgress(url, formData, headers, { onProgress, timeoutMs = UPLOAD_TIMEOUT_MS } = {}) {
...
export async function uploadSlideImage(file, opts = {}) {
  ...
  return uploadWithProgress(url, formData, headers, { onProgress: opts.onProgress, timeoutMs: UPLOAD_TIMEOUT_MS });
}
