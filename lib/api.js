...
/** Attempts per request (1 initial + retries). Meta CAPI uses its own retry in sendMetaCapi — not wrapped here. */
const MAX_API_ATTEMPTS = 3;

/**
 * Our timeout uses AbortController.abort(DOMException 'TimeoutError'). Fetch rejects with AbortError
 * whose cause is that DOMException — not err.name === 'TimeoutError'.
 */
function isApiTimeoutError(err) {
  if (!err) return false;
  if (err.name === 'TimeoutError') return true;
  if (err.name === 'AbortError') {
    if (err.cause && typeof err.cause === 'object' && err.cause.name === 'TimeoutError') return true;
    if (/request timed out after \d+ms/i.test(String(err.message || ''))) return true;
  }
  return false;
}

function isMetaCapiPath(pathOrUrl) {
  try {
    if (typeof pathOrUrl !== 'string') return false;
    if (pathOrUrl.startsWith('http')) {
      return new URL(pathOrUrl).pathname.endsWith('/analytics/meta-capi');
    }
    const p = pathOrUrl.startsWith('/') ? pathOrUrl : `/${pathOrUrl}`;
    return p === '/analytics/meta-capi' || p.endsWith('/analytics/meta-capi');
  } catch {
    return false;
  }
}

function isRetryableApiFailure(err, pathForCheck) {
  if (isMetaCapiPath(pathForCheck)) return false;
  if (!err) return false;
  if (isApiTimeoutError(err)) return true;
...
async function performApiRequestOnce(path, options = {}) {
...
/**
 * JSON API to backend. Retries transient failures (429, 5xx, timeouts, network) up to MAX_API_ATTEMPTS.
 * Does not auto-retry `/analytics/meta-capi` (use sendMetaCapi). Pass `{ retry: false }` to disable.
 */
export async function apiRequest(path, options = {}) {
  const { signal: userSignal, retry: retryOpt = true, ...fetchRest } = options;
  const normalizedPath = path.startsWith('http') ? path : path.startsWith('/') ? path : `/${path}`;
  const skipRetry = retryOpt === false || isMetaCapiPath(normalizedPath) || (path.startsWith('http') && isMetaCapiPath(path));
  const maxAttempts = skipRetry ? 1 : MAX_API_ATTEMPTS;
  const pathForRetry = path.startsWith('http') ? path : normalizedPath;
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      return await performApiRequestOnce(path, { signal: userSignal, ...fetchRest });
    } catch (e) {
      lastErr = e;
      if (attempt === maxAttempts - 1 || !isRetryableApiFailure(e, pathForRetry)) throw e;
...
/** Admin: list / edit shop product categories (requires admin JWT). */
export async function getAdminCategories() {
  return api.get('/admin/categories');
}

export async function postAdminCategory(data) {
  return api.post('/admin/categories', data);
}

export async function patchAdminCategory(id, data) {
  return api.patch(`/admin/categories/${encodeURIComponent(id)}`, data);
}

export async function deleteAdminCategory(id) {
  return api.delete(`/admin/categories/${encodeURIComponent(id)}`);
}
...
/** Visitor analytics — transient failures retried inside apiRequest. */
...
/** Admin: delete all visitor analytics event rows. */
export async function deleteAdminAnalyticsEventsAll() {
  return api.delete('/admin/analytics/events');
}

/** Admin: remove Meta campaign/adset/ad IDs from every analytics row (rows kept). */
export async function postAdminMetaClearAllAttribution() {
  return api.post('/admin/analytics/meta-campaigns/clear-all-attribution', {});
}
...
/** Meta CAPI relay → same-origin `/analytics/meta-capi` only (no MPC / run.app URL in this repo). */
...
export async function approveAllPendingReviewsForProduct(productId) {
  return api.post(`/admin/reviews/product/${encodeURIComponent(productId)}/approve-all-pending`, {});
}

/** Admin: bulk pending customer reviews — body `{ reviews: [{ name, review, rating }] }` or pass array as `reviews` only */
export async function bulkImportPendingReviews(productId, reviewsPayload) {
  const body = Array.isArray(reviewsPayload) ? { reviews: reviewsPayload } : reviewsPayload;
  return api.post(`/admin/reviews/product/${encodeURIComponent(productId)}/bulk-import-pending`, body);
}
...
/** Admin-only: list upload zones (`products`, `blog`, `slider`, `reviews`). */
export async function getAdminUploadZones() {
  return api.get('/admin/uploads/zones');
}

/** Admin-only: list files in a zone with reference counts. */
export async function getAdminUploadsList(zone) {
  return api.get(`/admin/uploads/${encodeURIComponent(zone)}`);
}

/** Admin-only: DB references for one file. */
export async function getAdminUploadRefs(zone, filename) {
  return api.get(`/admin/uploads/${encodeURIComponent(zone)}/${encodeURIComponent(filename)}`);
}

/** Admin-only: delete file on disk. Fails with 409 if referenced unless `force` is true. */
export async function deleteAdminUploadFile(zone, filename, { force = false } = {}) {
  const q = force ? '?force=true' : '';
  return api.delete(`/admin/uploads/${encodeURIComponent(zone)}/${encodeURIComponent(filename)}${q}`);
}

/** Admin-only: delete many files in one zone. Returns `{ deleted, skipped }`. */
export async function bulkDeleteAdminUploadFiles(zone, filenames, { force = false } = {}) {
  return api.post(`/admin/uploads/${encodeURIComponent(zone)}/bulk-delete`, { filenames, force });
}
