...
/** First `<ul>` after an h3 whose text is "Helps With" or "Key Benefits" (desktop PDP editorial). */
export function extractHelpsWithLines(html) {
  if (!html || typeof html !== 'string') return [];
  const head = /<h3[^>]*>([\s\S]*?)<\/h3>\s*<ul>([\s\S]*?)<\/ul>/gi;
  let m;
  while ((m = head.exec(html)) !== null) {
    const title = m[1].replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
    if (!/helps\s*with|key\s*benefits/i.test(title)) continue;
    const items = [];
    const liRe = /<li[^>]*>([\s\S]*?)<\/li>/gi;
    let x;
    while ((x = liRe.exec(m[2])) !== null) {
      const text = x[1].replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
      if (text) items.push(text);
    }
    return items;
