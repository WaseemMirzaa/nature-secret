...
/** Fetch products and categories from API; returns { products, categories, loading, fromApi }. Always fetches from API; uses store only as initial/fallback. */
export function useProductsAndCategories(storeProducts, storeCategories = FALLBACK_CATEGORIES) {
  ...
  const [categories, setCategories] = useState(Array.isArray(storeCategories) ? storeCategories : FALLBACK_CATEGORIES);
    ...
    Promise.all([getProducts({ limit: 200 }).catch(() => null), getCategories().catch(() => null)])
      .then(([productsRes, categoriesRes]) => {
        ...
        if (categoriesRes != null) {
          setCategories(categoryList.length ? categoryList : (Array.isArray(storeCategories) ? storeCategories : FALLBACK_CATEGORIES));
          setFromApi(true);
        } else {
          setCategories(Array.isArray(storeCategories) ? storeCategories : FALLBACK_CATEGORIES);
        }
      })
      .catch(() => {
        if (!cancelled) {
...
/** Fetch blog posts from API; returns { posts, loading, fromApi }. Always fetches from API; uses store only as initial/fallback. */
export function useBlogPosts(storePosts = []) {
  ...
  useEffect(() => {
    ...
    getBlogPosts({ limit: 100 })
      ...
      .catch(() => {
        if (!cancelled) setPosts(Array.isArray(storePosts) ? storePosts : []);
      })
      .finally(() => {
        if (!cancelled) setLoading(false);
      });
