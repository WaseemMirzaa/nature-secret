...
import { mergeDuplicateCartLines } from '@/lib/cartLine';
...
function cartVariantKey(variantId) {
...
export const useCartStore = create(
  persist(
    (set) => ({
      ...
      mergeDuplicateLines: () =>
        set((state) => ({
          items: mergeDuplicateCartLines(state.items),
        })),
      addItem: (item) =>
        set((state) => {
          const ivk = cartVariantKey(item.variantId);
          const existing = state.items.find(
            (i) => i.productId === item.productId && cartVariantKey(i.variantId) === ivk
          );
          const next = existing
            ? state.items.map((i) =>
      ...
      updateQty: (productId, variantId, qty) =>
        set((state) => {
          const vk = cartVariantKey(variantId);
          return {
            items:
              qty < 1
                ? state.items.filter(
                    (i) => !(i.productId === productId && cartVariantKey(i.variantId) === vk)
                  )
                : state.items.map((i) =>
                    i.productId === productId && cartVariantKey(i.variantId) === vk ? { ...i, qty } : i
                  ),
          };
        }),
    ...
    {
      name: CART_KEY,
      /** Session-scoped: cart clears when the browser session ends (avoids stale multi-day carts / 2× checkout totals). */
      storage: createJSONStorage(() => sessionStorage),
      partialize: (state) => ({ items: state.items }),
      /**
       * If the user taps Add to cart before persist rehydration finishes, merge stored + in-memory lines
       * instead of letting storage overwrite the new line.
       */
      merge: (persistedState, currentState) => {
        const pi = Array.isArray(persistedState?.items) ? persistedState.items : [];
        const ci = Array.isArray(currentState?.items) ? currentState.items : [];
        const items = ci.length > 0 ? mergeDuplicateCartLines([...pi, ...ci]) : pi;
