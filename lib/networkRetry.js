 ...
 * Each failure increments the counter once (see `__NS_PAGE_RELOAD_QUEUED__` guard in React paths).
 */
...
/** Stale HTML vs dev server / deploy: chunks or CSS under `/_next/static/` 404 or abort. */
export function isNextStaticAssetRecoverableMessage(msg) {
  if (typeof msg !== 'string' || !msg) return false;
  if (isGenericUserAbortMessage(msg)) return false;
  const s = msg.toLowerCase();
  if (!s.includes('/_next/static/')) return false;
  return (
    s.includes('failed to load') ||
    s.includes('404') ||
    s.includes('not found') ||
    s.includes('net::err') ||
    s.includes('err_aborted') ||
    s.includes('chunk') ||
    s.includes('failed to fetch')
  );
}
...
export function isRecoverablePageLoadError(message) {
  ...
  return (
    ...
    isNextStaticAssetRecoverableMessage(s)
  );
}
...
/** Inline script (layout) — must stay ES5-safe, no imports. Progress + reload until max; then Try again only. */
export function networkRetryInlineScript() {
  ...
  function isNextStaticStr(m){if(!m||isGenAbort(m))return false;var s=String(m).toLowerCase();if(s.indexOf('/_next/static/')===-1)return false;return s.indexOf('failed to load')!==-1||s.indexOf('404')!==-1||s.indexOf('not found')!==-1||s.indexOf('net::err')!==-1||s.indexOf('err_aborted')!==-1||s.indexOf('chunk')!==-1||s.indexOf('failed to fetch')!==-1;}
  function isNextStaticTarget(e){var t=e&&e.target;if(!t)return false;var u=(t.tagName==='SCRIPT'&&t.src)||(t.tagName==='LINK'&&t.href)||'';return String(u).indexOf('/_next/static/')!==-1;}
  ...
  function recoverable(m){return isChunk(m)||isNet(m)||isSrv(m)||isNextStaticStr(m);}
  function showExhausted(){
    try{
      document.body.innerHTML='<div style="min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:24px;padding:24px;background:#fafaf9"><p style="max-width:24rem;text-align:center;font-size:14px;color:#57534e;margin:0">We could not load the page after several tries. Check your connection, then try again.</p><button type="button" id="ns-pl-retry-btn" style="border:0;border-radius:9999px;background:#171717;color:#fff;padding:12px 32px;font-size:14px;font-weight:600;cursor:pointer">Try again</button></div>';
      var btn=document.getElementById('ns-pl-retry-btn');
  ...
  function showLoader(cnt,m){
    try{
      if(!document.body)return;
      var pct=Math.round(Math.min(cnt,m)/m*100);
      document.body.innerHTML='<div style="min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:24px;padding:24px;background:#fafaf9" aria-busy="true"><div style="width:40px;height:40px;border:2px solid #e7e5e4;border-top-color:#1c1917;border-radius:9999px;animation:ns-spin 0.8s linear infinite"></div><div style="width:100%;max-width:20rem"><div style="height:6px;border-radius:9999px;background:#e7e5e4;overflow:hidden"><div style="height:100%;border-radius:9999px;background:#171717;width:'+pct+'%"></div></div><p style="margin:8px 0 0;font-size:12px;font-weight:500;color:#57534e;text-align:center">Reconnecting... '+cnt+' / '+m+'</p></div><style>@keyframes ns-spin{to{transform:rotate(360deg)}}</style></div>';
    }catch(e){}
  }
  function reload(){
    try{
  ...
  window.addEventListener('error',function(e){
    ...
    if(recoverable(msg)||isNextStaticTarget(e))reload();
  });
