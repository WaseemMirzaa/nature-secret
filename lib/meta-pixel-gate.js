 ...
 * and traffic looks Meta (fbclid, utm, referrer). PDP/checkout landings are common for ads.
 */
...
/**
 * First paint path that should open the Meta gate with Meta traffic (not blog, etc.).
 * Home, product detail `/shop/:id`, and checkout funnels.
 */
...
export function isMetaPixelEnabledForSession() {
  ...
  try {
    ...
    try {
      ...
      open = isEligibleFirstLandingPath(snap.path) && isMetaTraffic(u, snap.ref || '');
    } catch {
      open = false;
    }
