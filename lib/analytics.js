...
import { isMetaPixelEnabledForSession, META_HEAD_PAGE_VIEW_EID_KEY } from '@/lib/meta-pixel-gate';
...
import { metaPdpEarlyVcSessionKey } from '@/lib/metaEarlyVc';
...
export function trackPageView(path = '') {
  ...
  let headPvEid = null;
  try {
    headPvEid = sessionStorage.getItem(META_HEAD_PAGE_VIEW_EID_KEY);
  } catch (_) {}
  ...
  if (firstLandingPair && headPvEid) {
    ...
    try {
      sessionStorage.removeItem(META_HEAD_PAGE_VIEW_EID_KEY);
    } catch (_) {}
  } else if (firstLandingPair && window.fbq) {
...
/**
 * When the pixel gate is open, wait for head-injected `fbq` / `fbevents` so ViewContent is not dropped.
 * @param {object|string} productOrId
 */
export function trackViewContentWhenReady(productOrId, value, currency = 'PKR') {
  if (typeof window === 'undefined') return;
  if (!isMetaPixelEnabledForSession()) {
    trackViewContent(productOrId, value, currency);
    return;
  }
  const run = () => trackViewContent(productOrId, value, currency);
  if (window.fbq) {
    run();
    return;
  }
  const onFbe = () => {
    window.removeEventListener('meta-fbevents-loaded', onFbe);
    run();
  };
  window.addEventListener('meta-fbevents-loaded', onFbe);
  queueMicrotask(() => {
    if (window.fbq) {
      window.removeEventListener('meta-fbevents-loaded', onFbe);
      run();
...
export function trackViewContent(productOrId, value, currency = 'PKR') {
  ...
  const catalogId =
    ...
    String(productOrId.slug != null ? productOrId.slug : productOrId.id != null ? productOrId.id : '')
      .trim()
      .slice(0, 128);
  const catId = metaCategoryId(productOrId);
  if (typeof window !== 'undefined' && productOrId?.id) {
    try {
      const earlyEid = sessionStorage.getItem(metaPdpEarlyVcSessionKey(productOrId.id));
      if (earlyEid && String(earlyEid).trim()) {
        sessionStorage.removeItem(metaPdpEarlyVcSessionKey(productOrId.id));
        useAnalyticsStore.getState().track('productView', { contentId: String(catalogId), value });
        if (isMetaPixelEnabledForSession()) {
          const categoryIdsEarly = catId ? [String(catId)] : [];
          const vcLinesEarly = catalogId ? metaStandardContentsLines([{ id: catalogId, quantity: numItems }]) : null;
          relayMetaCapi({
            eventName: 'ViewContent',
            eventId: String(earlyEid).trim().slice(0, 128),
            eventSourceUrl: metaEventSourceUrl(),
            currency: cur,
            value: val,
            numItems,
            contentIds: catalogId ? [String(catalogId)] : [],
            categoryIds: categoryIdsEarly,
            ...(vcLinesEarly ? { contents: vcLinesEarly } : {}),
          });
        }
        return;
      }
...
export function trackAddToCart(product, value, quantity = 1, currency = 'PKR') {
  ...
  if (typeof window !== 'undefined') {
    ...
    const pixelId =
      typeof process !== 'undefined' ? String(process.env.NEXT_PUBLIC_META_PIXEL_ID || '').trim() : '';
    if (isMetaDebugEnabled()) {
    ...
    /** Commerce pixel: do not require session gate (explicit user action; head may omit fbq when gate closed). */
