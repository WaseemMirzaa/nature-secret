/**
 * Push notifications: VAPID/Web Push code commented out.
 * TODO: Replace with Firebase FCM when FCM key is provided.
 */
...
export class PushService {
  ...
  private vapidReady = false; // disabled until Firebase FCM
  ...
  constructor() {
    ...
    this.load();
    // VAPID commented out; use Firebase FCM later
  ...
  getVapidPublicKey(): string | null {
    ...
    return null; // disabled until Firebase FCM
  }

  addSubscription(_sub: { endpoint: string; keys: { p256dh: string; auth: string }; label?: string }) {
    // VAPID subscription storage commented; use FCM tokens later
    // const existing = this.subscriptions.findIndex((s) => s.endpoint === sub.endpoint);
    // const record: PushSubscriptionRecord = { endpoint: sub.endpoint, keys: sub.keys, label: sub.label, createdAt: new Date().toISOString() };
    // if (existing >= 0) this.subscriptions[existing] = record; else this.subscriptions.push(record);
    // this.save();
  }

  removeSubscription(_endpoint: string) {
  ...
  async notifyNewOrder(order: { id: string; total?: number; customerName?: string | null }) {
    // TODO: send via Firebase FCM when key is provided
    void order;
  ...
  async sendToAll(_payload: { title: string; body?: string; icon?: string; url?: string }) {
    // VAPID webpush commented; use Firebase FCM later
    if (!this.vapidReady || this.subscriptions.length === 0) return;
    // const payloadStr = JSON.stringify(payload);
    // const results = await Promise.allSettled(this.subscriptions.map((sub) => webpush.sendNotification({ endpoint: sub.endpoint, keys: sub.keys }, payloadStr, { TTL: 60 })));
    // ...
  }
}
