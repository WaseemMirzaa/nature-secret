#!/usr/bin/env bash
# Cron script: restart backend if /health is down. Starts with node run-with-restart.js (restarts on crash).
...
nohup node run-with-restart.js >> /tmp/backend-start.log 2>&1 &
disown
